...
def cancel_node(state: State) -> State:
    ...
    # Get additional inputs based on arrival status and who cancelled
    wait_time = None
    cancellation_time = None
    
    if arrived:
        wait_time = get_wait_time()
    elif who_cancelled == "rider":
        cancellation_time = get_cancellation_time()
    ...
    # Build response based on cancellation type
...
- Driver arrived: {'Yes' if arrived else 'No'}"""
    ...
    # Only add distance if relevant (driver arrived or rider cancelled)
    if arrived or who_cancelled == "rider":
        response += f"\n- Distance from pickup: {distance_from_pin}m"
    
    # Add wait time and cancellation time if applicable
    if wait_time:
